...
def CommandJoke(message):
    ...
    keyboard.add(*[telebot.types.InlineKeyboardButton(text=smile, callback_data='like')])
...
def liks(c):
    if c.data == 'like':
        bot.send_message(c.message.chat.id, helpText)
...
def sendGtmIm(message):
    try:
        ...
        imgName = "src/img/request/" + translet(message_text) + translet(" " + message.from_user.first_name) + translet(
            " " + message.from_user.last_name) + ".jpg"
        ...
        bot.send_message(message.chat.id,
                         "Неизвестная ошибка, возможно вы не ввели запрос или сервер с картинками не доступен")


@bot.message_handler(content_types=['text'])  # Реакции на текст
def textconfig(message):
    erorText = "Я не понимаю, введи /help " + smile
